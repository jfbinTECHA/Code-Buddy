...
let McpRegistryService = McpRegistryService_1 = class McpRegistryService {
    ...
    discoverTools() {
        ...
        for (const [rootModule, mcpModules] of pairs) {
            this.logger.debug(`Discovering tools, resources, and prompts for module: ${rootModule.name}`);
    ...
    discoverToolsForModuleSubtree(mcpModuleId, modules) {
        ...
        const discovered = { tools: [], resources: [], prompts: [] };
        ...
        this.logger.debug(`Discovered tools: ${discovered.tools.length ? discovered.tools.join(', ') : 'none'}`);
        this.logger.debug(`Discovered resources: ${discovered.resources.length ? discovered.resources.join(', ') : 'none'}`);
        this.logger.debug(`Discovered prompts: ${discovered.prompts.length ? discovered.prompts.join(', ') : 'none'}`);
    }
